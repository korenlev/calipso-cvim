...
from scan.fetchers.api.api_fetch_projects import ApiFetchProjects
from scan.test.fetch.api_fetch.test_data.api_fetch_projects import *
...
class TestApiFetchProjects(TestFetch):
    ...
    def test_get_for_region(self):
        ...
        result = self.fetcher.get_tenants_for_region(self.region, TOKEN)
    ...
    def check_get_result(self, projects_for_api_user,
        ...
        original_method = self.fetcher.get_tenants_for_region
        # mock
        self.fetcher.get_tenants_for_region = MagicMock(return_value=region_result)
        self.fetcher.v2_auth_pwd = MagicMock(return_value=token)

        result = self.fetcher.get(PROJECT_ID)

        self.fetcher.get_tenants_for_region = original_method
