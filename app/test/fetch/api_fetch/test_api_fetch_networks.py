...
class TestApiFetchNetworks(TestFetch):
    ...
    def test_get(self):
        # store original get_networks method
        original_method = self.fetcher.get_networks
        # mock the get_networks method
        self.fetcher.get_networks = MagicMock(return_value=GET_FOR_REGION)

        result = self.fetcher.get(REGION_NAME)

        # reset get_networks method
