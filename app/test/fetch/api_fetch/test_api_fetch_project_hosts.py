...
from test.fetch.regions import *
...
class TestApiFetchProjectHosts(TestFetch):
    ...
    def test_add_host_type_with_nonexistent_type(self):
        ...
        self.assertIn(NONEXISTENT_TYPE, HOST_DOC["host_type"], "Can't put nonexistent type in host_type")
    ...
    def test_add_compute_host_type(self):
        ...
        self.assertEqual(HOST_DOC['zone'], HOST_ZONE, "Can't update zone name for compute node")
    ...
    def test_fetch_compute_node_ip_address(self):
        ...
        self.fetcher.fetch_compute_node_ip_address(HOST_TO_BE_FETCHED_IP, HOST_TO_BE_FETCHED_IP["host"])
        self.assertIn("ip_address", HOST_TO_BE_FETCHED_IP, "Can't update ip address for the compute host")
    ...
    def test_fetch_network_node_details(self):
        ...
        NETWORK_NODE_DOC = [doc for doc in HOSTS_TO_BE_FETCHED_NETWORK_DETAILS if doc['host'] == HOST_NAME][0]
        # check if the network node document has been updated
        self.assertIn("Network", NETWORK_NODE_DOC['host_type'], "Can't put Network in the network node host_type")
    ...
    def test_get_host_details(self):
        ...
        self.assertIn("Controller", result['host_type'], "Can't put controller type in the compute node host_type")

    def test_get_hosts_from_az(self):
        result = self.fetcher.get_hosts_from_az(AVAILABILITY_ZONE)
        self.assertNotEqual(result, [], "Can't get hosts information from availability zone")
    ...
    def test_get_for_region_with_error_availability_response(self):
        ...
        result = self.fetcher.get_for_region(self.region, TOKEN)
        self.assertEqual(result, [], "Can't get empty array when the availability zone response is incorrect")
    ...
    def test_get_for_region_with_error_hypervisors_response(self):
        ...
        result = self.fetcher.get_for_region(self.region, TOKEN)
        self.assertNotEqual(result, [], "Can't get hosts information when the hypervisors response is incorrect")

    def test_get(self):
        # store original get_for_region method
        original_method = self.fetcher.get_for_region
        # mock the data for get_for_region method
    ...
    def test_get_with_wrong_project_name(self):
        ...
        self.assertEqual(result, [], "Can't get empty array when the project name is not admin")
