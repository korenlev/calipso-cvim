...
def get_args():
    ...
    parser.add_argument("-W", "--warning", nargs="?",
        ...
        default='1%/300/600',
        help="warning thresholds: packet-loss (%)/avg-rtt (ms)/max-rtt (ms)" +
        "(example: 1%/300ms/600ms)")
    parser.add_argument("-C", "--critical", nargs="?",
        type=thresholds_string,
        default='10%/1000/2000',
        help="critical thresholds: packet-loss (%)/avg-rtt (ms)/max-rtt (ms)" +
        "(example: 1%/300ms/600ms)")
    ...
    parser.add_argument("-p", "--pattern", nargs="?", type=str, default='OS-DNA',
        help="pattern to pad packet with")
    ...
    parser.add_argument("-s", "--packetsize", nargs="?", type=int, default=256,
        help="size of packet vseconds to wait for completion of all responses")
...
try:
    ...
    print("Error doing ping: " + binary2str(e.output) + "\n")
    exit(2)
...
if rc < 2:
    ...
    if packet_loss > packet_loss_critical or \
                    avg_rtt >= avg_rtt_critical or \
                    max_rtt >= max_rtt_critical:
        rc = 2
    elif packet_loss > packet_loss_warn or \
                    avg_rtt >= avg_rtt_warn or \
