...
class CliFetchInstanceVnics(CliAccess):
    ...
    def get(self, id, enable_cache=True):
        ...
        if "Compute" not in host["host_type"]:
            return []
        lines = self.run_fetch_lines("virsh list", instance["host"], enable_cache=enable_cache)
        ...
        for id in virsh_ids:
            results.extend(self.get_vnics_from_dumpxml(id, instance, enable_cache=enable_cache))
        return results

    def get_vnics_from_dumpxml(self, id, instance, enable_cache=True):
        xml_string = self.run("virsh dumpxml " + id, instance["host"], enable_cache=enable_cache)
