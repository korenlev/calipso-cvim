...
if [[ "$1" = "enable" ]]; then
    ...
    docker exec -i ${scan_container} wget http://mirror.centos.org/centos/7/os/x86_64/Packages/openssh-server-7.4p1-16.el7.x86_64.rpm
    docker exec -i ${scan_container} rpm -i tcp_wrappers-libs-7.6-77.el7.x86_64.rpm
    docker exec -i ${scan_container} rpm -i openssh-server-7.4p1-16.el7.x86_64.rpm
