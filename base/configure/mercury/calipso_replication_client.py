...
class MongoConnector(object):
    ...
    def get_collection(self, db_name, collection):
        file_name = "%s-%s.json" % (db_name, collection)
        ...
        for document in cursor:
            ...
            if num_max == 1:
                ...
                f.write(json.dumps(document, indent=4, sort_keys=False, default=json_util.default))
        f.write(']')
        print ("exported data saved to {}/{}-{}.json".format(BACKUP_DATA_PATH, db_name, collection))
        return f

    def insert_collection(self, db_name, collection):
        file_name = "%s-%s.json" % (db_name, collection)
...
if __name__ == "__main__":
    DBN = input("How many DBs to replicate? ")
    DB = {}
    DBs = []
    n = 1
    while n < DBN + 1:
        ...
        DB.update({"name": remote_name, "secret": remote_secret})
        DBs.append(DB)
        n += 1
    collection_names = ["inventory", "links", "messages", "scans", "scheduled_scans",
                        "statistics", "cliques"]
    local_name = raw_input("Central Calipso Server Hostname/IP\n")
    local_secret = raw_input("Central Calipso Server Secret\n")
    attempt = 2
    while True:
        try:
            # read from remote DBs and export to local json files
            for d in DBs:
                mongo_connector = MongoConnector(d["name"], DEFAULT_PORT,
                                                 DEFAULT_USER, d["secret"], AUTH_DB)
                print ("Exporting collections from {}...".format(DB["name"]))
                for col in collection_names:
                    print("Exporting Collection {}...".format(col))
                    mongo_connector.get_collection(d["name"], col)
                    mongo_connector.disconnect()
                    # write from all local json files into the central DB
                    mongo_connector = MongoConnector(local_name, DEFAULT_PORT,
                                                     DEFAULT_USER, local_secret, AUTH_DB)
                    print ("Importing data from {}-{}.json...".format(d["name"], col))
                    mongo_connector.insert_collection(d["name"], col)
            break
        except:
            traceback.print_exc()
